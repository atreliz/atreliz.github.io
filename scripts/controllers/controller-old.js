...
//---Grapth
  ...
  .controller('example5Ctrl', function ($scope) {
	...
	//DEFAULT VALUES TO PAINT
		var dataInit = {
				nodes: [
	              { name: "N1", id:"1", val:10, color:"blue", x:239, y:86,  fixed:true},
	              { name: "N2", id:"2", val:5,  color:"green", x:20,  y:20,  fixed:true},
	              { name: "N3", id:"3", val:20, color:"red", x:139, y:196, fixed:true}

				],
				edges: [
	              { source: 1, target: 2 , color:"black", width:"1",shape:"10,10"},
	              { source: 1, target: 0 , color:"red",   width:"5",  shape:"0,0"}
				]
			};


		//Load it on view
		$scope.GrapthSize={w:400,h:300};
		//$scope.Grafo=dataInit;
		...
		/*function that draws*/
		//var svg = d3.select("#svgContentD3").append("svg");
		
		var svg = d3.select("#svgContentD3 svg");

		$scope.resizeGraph=function(){
				    	
			var w = $scope.GrapthSize.w,
      			h = $scope.GrapthSize.h;
                ...
                console.log("resizeGraph DONE: W:"+w+" H:"+h);

		}


		 var getAttr = function(d) { console.log(d); return d }

	    $scope.reloadGraph=function(dataSet){
	    	...
	    	console.log("reload Graph CALL");
	    	//console.log(GrafoData);

	    	//var dataSet=$scope.Grafo
	    	//$scope.Grafo=GrafoData;
	    	...
	    	//var dataSet= $scope.Grafo,
	    		var w = $scope.GrapthSize.w,
      			...
      			var x = d3.time.scale()
				    .range([0, w])
				    .domain(d3.extent(dataSet, getAttr));
			...
			/*var node_drag = d3.behavior.drag()
	         ...
	         var node = svg.selectAll("circle")
				...
				//.attr("r", 4.5);
				//.call(node_drag);  This is how I remove the drag and dorp
	...
	//INTERACTION
		...
		/*Refresh Size*/
		$scope.changeGrafoSize = function(ChangedData) {
	    	console.log("GrapthSize CHANGED");
			$scope.resizeGraph();
	    };

		/*Refresh Data*/
	    $scope.changeGrafoData = function(ChangedData) {
	    	//$scope.Grafo = JSON.parse(ChangedData, null, 1);
	    	var newData=JSON.parse(ChangedData, null, 1);
	    	console.log(newData);
	    	$scope.reloadGraph(newData);
	    };

	    $scope.changeTextarea = function(ChangedData) {
	    	//$scope.Grafo = JSON.parse(ChangedData, null, 1);
	    	var newData=JSON.parse(ChangedData, null, 1);
	    	$scope.Grafo=newData;

	    	console.info("Textarea Changed");
	    ...
	    //INIT GRAPHT
	    $scope.resizeGraph(); //Size
	    $scope.Grafo=dataInit;
	    $scope.reloadGraph(dataInit); //Data

	    $scope.GrafoSTR=JSON.stringify(dataInit, null, 1);
